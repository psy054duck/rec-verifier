...
class Recurrence:
    ...
    def __init__(self, inits: dict[sp.Symbol, sp.Expr], conditions: list[Boolean],
                 transitions: list[dict[sp.Symbol, sp.Expr]], ind_var=sp.Symbol('_n', integer=True),
                 acc_transitions=None, e_transitions=None, bounded_vars=None):
    ...
    def _combine_branches(self):
        ...
        for i in range(len(self.conditions)):
            ...
            if (self.acc_transitions is None and trans1 in new_transitions) or (
                    trans1 in new_transitions and self.acc_transitions is not None and acc1 in new_acc_transitions): continue
            new_condition = cond1
            for j in range(i + 1, len(self.conditions)):
                ...
                if (self.acc_transitions is None and trans2 in new_transitions) or (
                        trans2 in new_transitions and self.acc_transitions is not None and acc2 in new_acc_transitions):
                    continue
    ...
    def solve_array(self):
        ...
        neg_scalar_closed_form_sp = scalar_closed_form.subs(
            {self.ind_var: self.ind_var - Recurrence.neg_ind_var - 1}).to_sympy()
        ...
        transitions_vars = self._t_transitions(neg_scalar_closed_form_sp)
        array_closed_forms = []
        for new_rec, t_list, acc, e, array_var in transitions_vars:
        ...
        return scalar_closed_form, array_closed_forms
    ...
    def _prepare_t(self):
        array_vars = [var for var in self.arity if self.arity[var] >= 1]
        t_list = [sp.Symbol('_t%d' % i, integer=True) for i in range(max(self.arity[var] for var in array_vars))]
        return t_list, array_vars

    def _t_transitions(self, scalar_closed_form):
        t_list, array_vars = self._prepare_t()
        res = []
        for array_var in array_vars:
            ...
            res.append((Recurrence({d: 0, acc: 0, e: 1} | self.inits, new_conditions, transitions,
                                   ind_var=Recurrence.neg_ind_var, acc_transitions=acc_transitions), t_list, acc, e,
                        array_var))
        return res
    ...
    def _expr2involving_t(self, t, t_expr, expr):
        ...
        intersect_vars = vars.intersection(vars_p)
        if len(intersect_vars) == 0:
            return expr
        var = list(intersect_vars)[0]
    ...
    def solve(self):
        ...
        while solver.check() == z3.sat:
            ...
            for i, (seq, k) in enumerate(zip(index_seq, ks + [sp.oo])):
                ...
                cur_closed_form = {var: cur_closed_form[var].subs(inits).subs(self.ind_var, self.ind_var - acc_k) for
                                   var in cur_closed_form}
                acc_k += k
            ...
            for i, k in enumerate(z3_ks):
                ...
                if i > 0:
                    ...
                    cur_closed_form = {var: sp.cancel(cur_closed_form[var].subs(self.ind_var, self.ind_var + acc_k)) for
                                       var in cur_closed_form}
                else:
                ...
                for j, s in enumerate(seq):
                    ...
                    shifted_closed_form = {
                        to_z3(var): z3.substitute(to_z3(cur_closed_form[var]), (z3_ind_var, shifted_ind_var)) for var in
                        cur_closed_form}
                    validation_cond = z3.substitute(to_z3(self.conditions[s]),
                                                    *[(var, shifted_closed_form[var]) for var in shifted_closed_form])
                    validation_cond = z3.ForAll(z3_ind_var,
                                                z3.Implies(z3.And(0 <= shifted_ind_var, shifted_ind_var < len(seq) * k),
                                                           validation_cond))
            ...
            if len(z3_ks) > 0:
                ...
                cur_closed_form = {var: sp.cancel(closed_forms[-1][var].subs(self.ind_var, self.ind_var + acc_k)) for
                                   var in closed_forms[-1]}
            else:
                cur_closed_form = {var: sp.cancel(closed_forms[-1][var]) for var in closed_forms[-1]}
            seq = index_seq[-1]
            for j, s in enumerate(seq):
                shifted_ind_var = len(seq) * self.ind_var + j
                shifted_closed_form = {to_z3(var): to_z3(cur_closed_form[var].subs(self.ind_var, shifted_ind_var)) for
                                       var in cur_closed_form}
                validation_cond = z3.substitute(to_z3(self.conditions[s]),
                                                *[(var, shifted_closed_form[var]) for var in shifted_closed_form])
                validation_cond = z3.ForAll(z3_ind_var,
                                            z3.Implies(z3.And(0 <= to_z3(shifted_ind_var)), validation_cond))
            ...
            constraint = [z3.substitute(c, *[(to_z3(var), to_z3(self.inits[var])) for var in self.inits]) for c in
                          constraint]
            ...
            for i in range(len(closed_forms)):
                closed_form = {
                    var: closed_forms[i][var].subs(subs_pairs1, simultaneous=True).subs(subs_pairs2, simultaneous=True)
                    for var in closed_forms[i]}
    ...
    def solve_with_inits(self, inits: dict[sp.Symbol, sp.Integer | int]):
        ...
        while l < BOUND:
            ...
            while cur_threshold != 0:
                ...
                if len(cur_periodic_seg) == 0:
                    cur_closed_form = [{v: closed_form[v].subs(values[0]) for v in closed_form} for closed_form in
                                       cur_closed_form]
                    non_periodic_res = [(self.ind_var < cur_threshold + smallest, cur_closed_form)] + non_periodic_res
                    break
                else:
                    cur_closed_form = [{v: closed_form[v].subs(values[cur_threshold]) for v in closed_form} for
                                       closed_form in cur_closed_form]
            ...
            validate_closed_form = [{v: closed_form[v].subs(values[threshold]) for v in closed_form} for closed_form in
                                    periodic_closed_form]
            cur_smallest = self.validate(validate_closed_form, periodic_seg)
            periodic_closed_form = [
                {v: closed_form[v].subs(values[threshold] | {self.ind_var: self.ind_var - threshold}) for v in
                 closed_form} for closed_form in periodic_closed_form]
            ...
            period = len(periodic_closed_form)
            cur_inits = {var: periodic_closed_form[cur_smallest % period][var].subs(self.ind_var, cur_smallest) for var in cur_inits}
    ...
    def validate(self, closed_form: list[dict[sp.Symbol, sp.Expr]], periodic_seg: list[int]):
        ...
        for i, cond in enumerate(self.conditions):
            for j, p in enumerate(periodic_seg):
                to_validate = cond.subs(Recurrence._transform_closed_form(closed_form[j], {
                    self.ind_var: len(periodic_seg) * self.ind_var + j}))
                ...
                if (i == p and to_validate != sp.S.true):
                    ...
                    N = z3.Int('N')
                    s.add(
                        z3.ForAll(z3_ind_var, z3.Implies(z3.And(0 <= z3_ind_var, z3_ind_var < N), to_z3(to_validate))))
    ...
    def solve_periodic(self, periodic_seg: list[int]):
        ...
        for i in range(len(mod_closed_form)):
            mod_closed_form[i] = {v: mod_closed_form[i][v].subs({self.ind_var: (self.ind_var - i) / len(periodic_seg)})
                                  for v in mod_closed_form[i]}
    ...
    def periodic_closed_form2sympy(self, closed_form: list[dict[sp.Symbol, sp.Expr]]):
        ...
        for var in self.variables:
            res[var] = sp.Piecewise(*([(closed[var], sp.Eq(self.ind_var % len(closed_form), i)) for i, closed in
                                       enumerate(closed_form)] + [(-1, True)]))
        return res
